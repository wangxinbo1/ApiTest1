...
# 读取excel
do_excel = HandleExcel(os.path.join(DATA_DIR, config_read_file.get_value("excel", "case_excel")), "add")
...
class TestRecharge(unittest.TestCase):
    ...
    def test_user_login(self, one_case):
        ...
        new_data = HandleContext.add_paramization(one_case["data"])
        logger.info("\n请求url为{0}\ndata为{1}".format(url_new, new_data))
        add = self.resp.sendRequests(method=one_case["method"], url=url_new, data=eval(new_data))  # 返回加标的响应对象

        logger.info("\n响应数据为{1}".format(url_new, add.text))
        try:
            ...
            do_excel.write_result(row=one_case["case_id"] + 1, column=7, result=add.text)
            logger.info("写入结果结束")
